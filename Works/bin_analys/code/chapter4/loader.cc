...
static int load_sections_bfd(bfd *bfd_h, Binary *bin)
{
  ...
  for(bfd_sec = bfd_h->sections; bfd_sec; bfd_sec = bfd_sec->next) {
    ...
    if(bfd_flags & SEC_CODE) {
      ...
      sectype = Section::SEC_TYPE_READONLY;
    } else if(bfd_flags & SEC_LOAD) {
      sectype = Section::SEC_TYPE_LOAD;
    } else if(bfd_flags & SEC_RELOC) {
      sectype = Section::SEC_TYPE_RELOC;
    } else if(bfd_flags & SEC_ROM) {
      sectype = Section::SEC_TYPE_ROM;
    } else if(bfd_flags & SEC_ALLOC) {
      sectype = Section::SEC_TYPE_ALLOC;
    } else if(bfd_flags & SEC_STRINGS) {
      sectype = Section::SEC_TYPE_STRINGS;
    } else if(bfd_flags & SEC_NEVER_LOAD) {
      sectype = Section::SEC_TYPE_NEVER_LOAD;
    } else if(bfd_flags & SEC_CONSTRUCTOR) {
      sectype = Section::SEC_TYPE_CONSTRUCTOR;
    } else if(bfd_flags & SEC_IS_COMMON) {
      sectype = Section::SEC_TYPE_IS_COMMON;
    } else {
      continue;
    }
